        ...
        <div className="rmdb-movieinfo"
            style={{
                background: props.movie.backdrop_path ?
                    `url('${IMAGE_BASE_URL}${BACKDROP_SIZE}${props.movie.backdrop_path}')` : '#000'
            }}
        >
            <div className="rmdb-movieinfo-content">
                ...
                <div className="rmdb-movieinfo-text">
                    ...
                    {props.directors.map((element, index) => {
                        return (
                            <p key={index} className="rmdb-director">{element.name}</p>
                        )
                    })}
                </div>
                <FontAwesome className="rmdb-fa-film" name="film" size="5x" />
            </div>
        </div>
    );
};
